		...
		if not deactivatedFlasks then
			...
			if container then
				...
				if fromPosition.x == CONTAINER_POSITION and container ~= storeInbox and container:getEmptySlots() ~= 0 then
					if player:getFreeCapacity() >= ItemType(potion.flask):getWeight() then
						container:addItem(potion.flask, 1)
					else
						player:sendTextMessage(MESSAGE_STATUS_WARNING, "You do not have enough capacity to carry the flask.")
					end
				else
					if player:getFreeCapacity() >= ItemType(potion.flask):getWeight() then
